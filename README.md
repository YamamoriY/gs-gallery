...
地図では色と破線の枠で区別し、シーンページにも推定である旨を出している。
...
- **20260902_692 と 20260902_693 の PLY が同一** — 成果物の 2 ファイルが
  完全に同じ内容だった (サイズと 5 か所の 64KB ブロックが一致)。
  旧名では 689 と 690 で、当時の変換ログでも点数が同じだった。
  どちらか一方の学習出力がもう一方で上書きされたか、コピーされたと思われる。
  **片方の木には固有の 3DGS が無い。** 一覧とシーンページに警告を出している。
  学習をやり直したら `scenes.overrides.json` の `warning` を消す。
